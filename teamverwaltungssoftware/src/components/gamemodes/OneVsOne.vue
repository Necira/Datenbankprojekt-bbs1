<template>
  ...
  <div class="one-vs-one">
    ...
    <div
      class="actions"
      v-if="!winner && playerOne && playerTwo && playerOne != playerTwo"
    >
      <button class="play-randomly" @click="setRandomizedWinner">
        Play Randomly
      </button>
      <div class="choose-winner">
        ...
        <button class="set-winner" @click="setWinner(chooseWinner)">
          Set Winner
        </button>
      </div>
    </div>
  ...
  methods: {
    ...
    async updateEloPoints(winner, loser) {
      try {
        const responseWinner = await fetch(
          `http://localhost:3000/getElo/${winner}`
        );
        const responseLoser = await fetch(
          `http://localhost:3000/getElo/${loser}`
        );
    ...
    async setWinner(winner) {
      if (
        ...
        const loser =
          winner === this.playerOne ? this.playerTwo : this.playerOne;

        try {
          const { newEloWinner, newEloLoser } = await this.updateEloPoints(
            winner,
            loser
          );
    ...
    async setRandomizedWinner() {
      if (
        ...
        this.message =
          "Please select both players and ensure they are different.";
      }
    },
